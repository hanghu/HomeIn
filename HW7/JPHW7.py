...
crimes = pd.read_csv('../data/King_County_Sheriff_s_Office.csv')
...
for name, row in houses[0:200].iterrows():
    folium.Marker([row["lat"], row["long"]],
                  popup="Price: {0}$,Size: {1}, Year Built: {2}"\
...
kccrimes_heatmap.add_children(
    plugins.HeatMap(
    	[[row["latitude"], row["longitude"],row["city"]]for iters, row in crimes[0:100].iterrows()],
